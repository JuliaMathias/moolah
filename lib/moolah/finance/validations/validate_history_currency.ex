defmodule Moolah.Finance.Validations.ValidateHistoryCurrency do
  @moduledoc """
  Ensures investment history values use the same currency as the parent investment.

  The validation loads the investment referenced by `investment_id` and compares the
  history `value` currency against the investment's `current_value` currency.
  ...
  ## Examples
      ...
      ...>   })
      iex> Moolah.Finance.Validations.ValidateHistoryCurrency.validate(changeset, [], %{})
      :ok
  """
  ...
  @impl true
  @doc """
  Validates that the history value currency matches the investment currency.
